...
async def chat(
    ...
    async def event_generator() -> AsyncGenerator[ServerSentEvent, None]:
        # 1.调用Agent服务发起聊天，将Agent事件实时转换为SSE流
        # (端点本身必须是异步生成器+response_class=EventSourceResponse，
        # 路由层才会对每个事件做SSE编码；否则返回的Response对象会走裸流式响应，对ServerSentEvent直接调encode崩溃)
        async for event in agent_service.chat(
                session_id=session_id,
                message=request.message,
                attachments=request.attachments,
                latest_event_id=request.event_id,
                timestamp=from_timestamp(request.timestamp) if request.timestamp else None,
        ):
    ...
    try:
        ...
        async with websockets.connect(sandbox_vnc_url) as sandbox_ws:
            ...
            # 8.等待任意任务结束意味WebSocket连接终端
                ...
                fs=[forward_task1, forward_task2],
