...
from typing import Type, Optional, AsyncGenerator, List, Callable
...
from app.domain.models.sesssion import Session, SessionStatus
...
class AgentService:
    ...
    async def _safe_update_unread_count(self, session_id: str) -> None:
        ...
        try:
            ...
            async with uow:
                await uow.session.update_unread_message_count(session_id, count=0)
    ...
    async def chat(
        ...
        try:
            ...
            # 3.是否携带有效人类消息（空 JSON / 空字符串视为续流请求）
            ...
            if has_message:
                ...
                # 空 JSON：仅对未完成任务做输出流续传（详情页重连 / 首页跳转接管）
                if task is None or task.done:
                    logger.info(f"会话[{session_id}]空请求且无进行中任务，结束续流")
                    return
                if session.status not in (SessionStatus.RUNNING, SessionStatus.PENDING):
                    logger.info(
                        f"会话[{session_id}]空请求，状态={session.status.value}，无需续流"
                    )
                    return
            ...
            while task and not task.done:
                # 12.从输出消息队列中获取数据
                # 有界阻塞(5000ms)：即便终端事件因异常丢失，也能在task.done后正常退出，
                # 避免block=0永久阻塞导致Redis读超时；task.done在run结束后必然为True。
                event_id, event_str = await task.output_stream.get(
                    start_id=latest_event_id, block_ms=5000
                )
                # 只有真正读到事件时才推进偏移量，防止空轮询把latest_event_id重置为None
                # (get会回退到"0")导致历史事件被重复读取、事件被重复下发。
                ...
                # 14.将未读消息数重置为0
                async with self._uow:
                    await self._uow.session.update_unread_message_count(session_id, count=0)
            ...
            # 注意：当SSE客户端断开连接时，sse_starlette使用anyio cancel scope取消当前Task
