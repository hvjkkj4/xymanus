...
GET_INTERACTIVE_ELEMENTS_FUNC = """() => {
  ...
  // 2.获取页面上所有可交互的元素，覆盖常见可点击、可输入、可聚焦节点
  const elements = document.querySelectorAll(
    'a[href], button, input, textarea, select, summary, [role="button"], [role="link"], [contenteditable="true"], [tabindex]:not([tabindex="-1"])',
  );
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    // 隐藏 input 不参与交互元素收集
    if (tagName === "input" && element.type === "hidden") continue;

    // 10.根据标签类型不同处理不同的逻辑，首先是输入框/文本域/下拉菜单
    if (["input", "textarea", "select"].includes(tagName)) {
      if (element.value) {
        text = String(element.value).trim();
      }
      ...
      if (tagName === "input") {
        ...
        // 13.查找父级或同级的label（当没有 for 属性绑定时）
        if (!labelText) {
...
# 在执行代码前先执行这段js代码，实现将console.log内容存储到window.console.logs中
