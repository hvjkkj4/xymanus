...
class DockerSandbox(Sandbox):
    ...
    def __init__(
        ...
        # 沙箱VNC WebSocket由noVNC/websockify提供(6080)，转发到x11vnc(5900)
        self._vnc_url = f"ws://{ip}:6080/websockify"
    ...
    def _get_container_ip(cls, container: Model) -> str:
        ...
        # 注意:新版Docker的NetworkSettings顶层可能没有IPAddress键(只有Networks里才有),
        # 必须用.get()获取,否则会抛KeyError: 'IPAddress'
    ...
    async def list_files(self, dir_path: str) -> ToolResult:
        """传递目录列出沙箱指定目录下的所有文件"""
        return await self.find_files(dir_path, glob_pattern="*")
