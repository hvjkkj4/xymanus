...
from app.domain.models.sesssion import Session
...
class SessionModel(Base):
    ...
    def update_from_domain(self, session: Session) -> None:
        ...
        注意：events/files/memories 由 add_event/add_file/save_memory 等专用方法
        以核心(Core)SQL直接追加更新，这里一律不覆盖。否则保存时会把会话里
        「领域模型加载时刻」的快照写回，把事务期间已追加的事件/文件/记忆清空。
        """
