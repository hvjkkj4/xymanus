...
from sqlalchemy import update, select, delete, func, cast
...
from app.domain.models.sesssion import Session, SessionStatus, SESSION_PLACEHOLDER_TITLES
...
class DBSessionRepository(SessionRepository):
    ...
    async def update_title(self, session_id: str, title: str) -> None:
        """更新会话标题：仅当当前仍为占位标题时写入，锁定首个正式标题"""
        normalized = (title or "").strip()
        if not normalized:
            return

        # 1.仅占位标题可被覆盖，避免跟进提问改写侧边栏会话名
        stmt = (
            update(SessionModel)
            .where(
                SessionModel.id == session_id,
                SessionModel.title.in_(tuple(SESSION_PLACEHOLDER_TITLES)),
            )
            .values(title=normalized)
        )
        result = await self.db_session.execute(stmt)

        # 2.未更新时区分「已锁定」与「会话不存在」
        if result.rowcount == 0:
            existing = await self.get_by_id(session_id)
            if existing is None:
                raise ValueError(f"会话[{session_id}]不存在，请核实后重试")
            # 标题已锁定为首个正式标题，忽略后续 TitleEvent
            return

    async def force_update_title(self, session_id: str, title: str) -> None:
        """强制更新会话标题（用于将错误的「最新提问」标题纠正为事件首个标题）"""
        normalized = (title or "").strip()
        if not normalized:
            return
        stmt = (
    ...
    async def update_latest_message(self, session_id: str, message: str, timestamp):
        """更新会话最新消息"""
        # 1.构建sql更新会话消息并执行
        stmt = (
            update(SessionModel)
