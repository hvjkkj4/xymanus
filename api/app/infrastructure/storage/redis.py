    ...
    async def init(self) -> None:
        ...
        try:
            ...
            self._client = Redis(
                ...
                # 显式关闭socket读超时：redis-py 8.x默认socket_timeout=5秒，会让阻塞式的
                # XREAD(block=5000)在Redis服务器阻塞期间误报 TimeoutError(实测约58秒才抛错)，
                # 导致chat端SSE流一直读不到事件。block参数本身已提供阻塞上限，无需socket超时。
                socket_timeout=None,
            )
            # 3.测试连接redis缓存
