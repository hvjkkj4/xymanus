...
from typing import Literal, Union, List, Any, Optional, Dict
...
class BaseEvent(BaseModel):
    ...
    type: Literal[""] = ""  # 事件的类型
    created_at: datetime = Field(default_factory=now)  # 事件创建时间（上海时区）
...
class BrowserToolContent(BaseModel):
    """浏览器工具扩展内容"""
    screenshot: str  # 浏览器快照截图(base64图片)
...
class ToolEvent(BaseEvent):
    ...
    tool_call_id: str  # 工具调用唯一id
    tool_name: str  # 工具箱/工具集名字（你之前写的BaseTool集合名称）
    tool_content: Optional[ToolContent] = None  # 工具额外扩展信息
    function_name: str  # LLM真正调用的函数名称
    function_args: Dict[str, Any]  # LLM输出的调用参数
    function_result: Optional[ToolResult] = None  # 工具返回结果
    status: ToolEventStatus = ToolEventStatus.CALLING  # 默认：调用中
