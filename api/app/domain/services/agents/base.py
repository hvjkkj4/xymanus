...
from app.domain.models.event import ToolEvent, ToolEventStatus, ErrorEvent, MessageEvent
...
class BaseAgent(ABC):
    ...
    async def _invoke_llm(self, messages: List[Dict[str, Any]], format: Optional[str]) -> Dict[str, Any]:
        """调用语言模型并处理记忆内容"""
        # 1.将消息添加到记忆中
    ...
    async def invoke(self, query: str, format: Optional[str] = None) -> AsyncGenerator[Dict[str, Any], None]:
        """传递消息+响应格式调用程序生成异步迭代内容"""
        # 1.需要判断下是否传递了format
        format = format if format else self._format

        # 2.调用语言模型获取响应内容。
        #   注意：工具调用轮次必须强制format=None，不能带response_format={"type":"json_object"}。
        #   DeepSeek上JSON Output与Function Calling互斥，工具轮次带json_object模型将永远
        #   不返回tool_calls。是否强制JSON由下方"最终答案"阶段按_format决定。
        ...
        for _ in range(self._agent_config.max_iterations):
            ...
            if not message or not message.get("tool_calls"):
                # 4.1 _invoke_llm所有重试都失败时会返回None，给出明确错误而不是对None调.get()崩溃
                if not message:
                    yield ErrorEvent(error="调用大语言模型失败，请稍后重试")
                    return
                break
            ...
            #    工具结果轮次同样不带json_object，模型可能继续调用工具
            message = await self._invoke_llm(tool_messages, format=None)
        else:
            # 13.超过最大迭代次数后，则抛出错误并终止
            yield ErrorEvent(error=f"Agent迭代超过最大迭代次数: {self._agent_config.max_iterations}, 任务处理失败")
            return

        # 14.最终答案阶段：若Agent要求json_object结构(且模型已停止调用工具)，
        #    确保最终输出是严格JSON。先检查自由输出是否已是JSON对象(零额外LLM调用)，
        #    否则用json_object模式重新生成一次——此时请求仍携带tools，但DeepSeek在
        #    json_object模式下强制输出纯JSON文本，正好符合最终结构化答案的需求。
        if format == "json_object" and not message.get("tool_calls"):
            content = (message.get("content") or "").strip()
            try:
                parsed = await self._json_parser.invoke(content)
                if not isinstance(parsed, dict):
                    raise ValueError("最终答案不是有效的JSON对象")
            except Exception:
                logger.info("最终答案不是纯JSON，使用json_object模式重新生成结构化输出")
                corrected = await self._invoke_llm(
                    messages=[{
                        "role": "user",
                        "content": "请基于下面的内容生成符合要求的纯JSON对象，只输出JSON本身，"
                                   "不要使用markdown代码块，不要输出任何解释或前缀文字：\n" + content,
                    }],
                    format="json_object",
                )
                if corrected:
