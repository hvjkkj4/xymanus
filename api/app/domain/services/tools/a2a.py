...
8.A2A客户端管理器停止时必须清除对应资源，涵盖了缓存，异步上下文管理器，避免资源泄露；
"""
...
class A2ATool(BaseTool):
    ...
    @tool(
        ...
        description="获取可远程调用的Agent卡片信息，包含Agent id、名称、描述、技能、请",
        parameters={},
        required=[]
    )
    ...
    @tool(
        ...
        parameters={
            "id": {
                "type": "string",
                "description": "需要调用远程agent的id，格式参考get_remote_agent_cards返回的数据结构"
            },
            "query": {
                "type": "string",
