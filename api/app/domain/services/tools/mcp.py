...
非常耗时，所以需要我们缓存工具的参数信息，只在初始化的时候才调用一次，
...
class MCPClientManager:
    ...
    async def _connect_mcp_servers(self) -> None:
        """根据配置连接所有MCP服务"""
        # 1.循环遍历传递进来的所有MCP服务器，跳过未启用的服务器
        # (enabled=false的服务器不建立连接，避免不可达的MCP服务拖慢任务启动)
        for server_name, server_config in self._mcp_config.mcpServers.items():
            ...
            try:
                ...
                # 3.MCP SDK在服务器连接失败(如超时)时会通过其内部anyio取消作用域对本任务
                # 投递CancelledError——这并非外部取消任务。按「单个MCP服务器连接失败」处理，
                # 记录日志并跳过该服务器。否则一次连接失败会把整个任务取消，导致后面的
                # Planner/ReAct流程与LLM调用永远无法执行(表现为chat 5秒后连接关闭)。
                logger.error(f"连接MCP服务器[{server_name}]被取消(连接超时/中断): {str(e)}")
                continue
            except Exception as e:
    ...
    async def _cache_mcp_server_tools(self, server_name: str, session):
    ...
    async def invoke(self, tool_name: str, arguments: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if not original_server_name or not original_tool_name:
                raise Exception(f"服务器解析MCP工具不存在: {tool_name}")

            # 7.获取该工具所属的会话
            session = self._clients.get(original_server_name)
            if not session:
                return ToolResult(success=False, message=f"MCP服务器[{original_server_name}]连接不存在")
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            # 清理期间被取消，忽略即可，状态照常清空
            logger.warning("清理MCP客户端管理器被取消，忽略")
        except Exception as e:
            # mcp SDK的streamable_http在连接超时/断开后会留下损坏的anyio取消作用域，
            # aclose()退出这些作用域时必然抛RuntimeError(cancel scope in a different task)。
            # 这是第三方mcp包的已知问题，只发生在清理阶段、不影响任务执行，记录后继续清空状态。
            logger.warning(f"清理MCP客户端管理器时MCP SDK报错(第三方包已知问题，可忽略): {str(e)}")
        finally:
            # 无论aclose成功与否都要清空本地缓存状态，避免资源泄露
            self._clients.clear()
            self._tools.clear()
