...
class ShellService:
        ...
        while True:
            ...
            if process.stdout:
                try:
                    ...
                    # 6.判断会话是否存在
                    shell_session = self.active_shells.get(session_id)
                    if shell_session:
                        # 7.更新会话输出和控制台记录
                        shell_session.output += output
                        if shell_session.console_records:
                            shell_session.console_records[-1].output += output
    ...
    async def wait_process(self, session_id: str, seconds: Optional[int] = None):
        """传递会话id+时间，等待子进程结束"""
        # 1.判断下传递的会话是否存在
    ...
    async def kill_process(self, session_id: str) -> ShellKillResult:
        ...
        try:
            ...
            # 9.记录日志并抛出异常
            logger.error(msg=f"关闭进程失败: {str(e)}", exc_info=True)
